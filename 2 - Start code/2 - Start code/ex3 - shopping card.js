...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach(element => {
    const perUnit = PRODUCTS.find((p) => p.id === element.id);
    result += perUnit.price * element.quantity;
  }
  );
...
function removeProductFromCart(productId) {
  // Write your code here
  let product = SHOPPING_CART.find((p) => p.id === productId);
  let productID = SHOPPING_CART.map((p) => p.id).indexOf(productId);
  if (product.quantity >= 2){
    product.quantity--;
  }
  else {
    SHOPPING_CART.splice(productID);
  }
